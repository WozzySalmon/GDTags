...
echo.
echo Usage:
echo   tools\windows\build_native.cmd [SCons args]
echo.
echo Defaults:
echo   platform=windows target=template_debug -j%%NUMBER_OF_PROCESSORS%%
echo.
echo Examples:
echo   tools\windows\build_native.cmd
echo   tools\windows\build_native.cmd target=template_release
echo   tools\windows\build_native.cmd -c
echo   tools\windows\build_native.cmd -j1 verbose=yes
exit /b 0
