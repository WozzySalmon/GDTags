...
func _run() -> void:
	...
	_assert(gameplay_tags.is_native_runtime_enabled(), "GameplayTags should select native runtime when the GDExtension is available")
	...
	_assert(container.is_class("NativeGameplayTagContainer"), "make_container should return NativeGameplayTagContainer when native is enabled")
	...
	_assert(query.is_class("NativeGameplayTagQuery"), "make_query_all should return NativeGameplayTagQuery when native is enabled")
